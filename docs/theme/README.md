...
Vuemerang has the main colors that are maintained throughout the application to facilitate the change and effective operation
...
We almost always need a personalized application with own colors. In Vuemerang, you can change the main colors to those that you need and the branding that your application has.

When starting **Vuemerang** tell it to use your colors. We add the new colors as parameters.
...
You can change the color of the application in process of execution with the function `$vm.theme` which as a parameter requires a json with the colors to change
...
By doing this, all **Vuemerang** components in the application that use a primary color with change.
...
@import '~vuemerang/dist/styles/index.styl'

$vm-primary = 50,100,200
